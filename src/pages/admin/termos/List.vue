<template>
  <under-construction />
  <!-- <page :breadcrumbs="[
    { label: 'Lista', link: '/termos' },
    ]">
    <q-btn data-cy="button" color="primary" icon="fa fa-plus" class="full-width" label="Inserir novo" to="/termos/inserir"/>
    <Table
      data-cy="table"
      label="Lista termos"
      :data="data"
  ...
  setup (_, { root }) {
    ...
    const functions = {
      ...
      onEdit (row: { id: number }) {
        return root.$router.push(`/termos/editar/${String(row.id)}`)
      },
      onDelete (row: { id: number }) {
        root.$q.dialog(confirmMsg).onOk(async () => {
          const { ok } = await api.delete(`termos/${String(row.id)}`)
