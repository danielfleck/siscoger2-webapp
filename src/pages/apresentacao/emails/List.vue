<template>
  <page :breadcrumbs="[
    { label: 'Lista', link: '/emails' },
    ]">
    <q-btn data-cy="button" color="primary" icon="fa fa-plus" class="full-width" label="Inserir novo" to="/emails/inserir"/>
    <Table
      data-cy="table"
      label="Lista emails"
      :data="data"
  ...
  setup (_, { root }) {
    ...
    const functions = {
      ...
      onEdit (row: { id: number }) {
        return root.$router.push(`/emails/editar/${String(row.id)}`)
      },
      onDelete (row: { id: number }) {
        root.$q.dialog(confirmMsg).onOk(async () => {
          const { ok } = await api.delete(`emails/${String(row.id)}`)
