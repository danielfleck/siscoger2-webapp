<template>
  <page :breadcrumbs="[
    { label: 'Lista', link: '/notas' },
    ]">
    <q-btn data-cy="button" color="primary" icon="fa fa-plus" class="full-width" label="Inserir novo" to="/notas/inserir"/>
    <Table
      data-cy="table"
      label="Lista notas"
      :data="data"
  ...
  setup (_, { root }) {
    ...
    const functions = {
      ...
      onEdit (row: { id: number }) {
        return root.$router.push(`/notas/editar/${String(row.id)}`)
      },
      onDelete (row: { id: number }) {
        root.$q.dialog(confirmMsg).onOk(async () => {
          const { ok } = await api.delete(`notas/${String(row.id)}`)
