<template>
  <page :breadcrumbs="[
    { label: 'Lista', link: '/links' },
    ]">
    <q-btn data-cy="button" color="primary" icon="fa fa-plus" class="full-width" label="Inserir novo" to="/links/inserir"/>
    <Table
      data-cy="table"
      label="Lista links"
      :data="data"
  ...
  setup (_, { root }) {
    ...
    const functions = {
      ...
      onEdit (row: { id: number }) {
        return root.$router.push(`/links/editar/${String(row.id)}`)
      },
      onDelete (row: { id: number }) {
        root.$q.dialog(confirmMsg).onOk(async () => {
          const { ok } = await api.delete(`links/${String(row.id)}`)
