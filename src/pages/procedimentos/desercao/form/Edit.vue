    ...
    <q-tab-panels v-model="tab" animated>
      <q-tab-panel name="main" class="row">
        ...
        <template v-if="register.id">
          ...
          <FileUpload label="Relatório do Encarregado" :data="{ proc: 'desercao', campo: 'relatorio_encarregado_file', id_proc: register.id}"/>
          <FileUpload label="Solução do Comandante" :data="{ proc: 'desercao', campo: 'solucao_cmt_file', id_proc: register.id}"/>
          <FileUpload label="Solução CMT Geral" :data="{ proc: 'desercao', campo: 'solucao_cmtgeral_file', id_proc: register.id}"/>
        </template>
      ...
      <q-tab-panel name="sobrestamentos" @transition="validateNavigation">
        <sobrestamento @submit="changeAndamento" :data="{ id_desercao: register.id }"/>
      </q-tab-panel>
  ...
  setup (_, { refs, root }) {
    ...
    async function changeAndamento (sobrestamento: { termino_data: string }) {
      ...
      if (!sobrestamento.termino_data) {
        vars.register.id_andamento = getSobrestamento('desercao')
        const { ok } = await api.put(`desercao/${id}`, vars.register, { silent: true })
        if (ok) return
      }
      vars.register.id_andamento = getAndamento('desercao')
      await api.put(`desercao/${id}`, vars.register, { silent: true })
    }
