<template>
  ...
  { label: 'Editar', link: '/exclusao/editar' },
  ]">
    <q-tabs
      v-model="tab"
      dense
    ...
    <q-tab-panels v-model="tab" animated>
      <q-tab-panel name="main" class="row">
        ...
        <div class="q-pa-md col-4">
          <Andamento v-model="register.id_andamento" type="exclusao"/>
        </div>
        <div class="q-pa-md col-4">
          <AndamentoCoger v-model="register.id_andamentocoger" type="exclusao"/>
        </div>
        ...
        <template v-if="register.id">
          <ProcedOrigem type="exclusao" :data="{ id_exclusao: register.id }"/>
          <Membro label="Sindicante" ref="sindicante" required :data="{ situacao: 'sindicante', id_exclusao: register.id }"/>
          <Membro label="Escrivão" ref="escrivao" :data="{ situacao: 'escrivao', id_exclusao: register.id }"/>
          <Acusado label="Sindicado" :data="{ situacao: 'sindicado', id_exclusao: register.id }"/>
          <Vitima :data="{ id_exclusao: register.id }"/>
          <FileUpload label="Relatório do Encarregado" :data="{ proc: 'exclusao', campo: 'relatorio_encarregado_file', id_proc: register.id}"/>
          <FileUpload label="Solução do Comandante" :data="{ proc: 'exclusao', campo: 'solucao_cmt_file', id_proc: register.id}"/>
          <FileUpload label="Solução CMT Geral" :data="{ proc: 'exclusao', campo: 'solucao_cmtgeral_file', id_proc: register.id}"/>
        </template>
        <q-btn @click="update" color="primary" label="Salvar" class="full-width"/>
      </q-tab-panel>

      <q-tab-panel name="movimentos">
        <movimento :data="{ id_exclusao: register.id }"/>
      </q-tab-panel>

      <q-tab-panel name="sobrestamentos" @transition="validateNavigation">
        <sobrestamento @submit="changeAndamento" :data="{ id_exclusao: register.id }"/>
      </q-tab-panel>

      <q-tab-panel name="arquivos" @transition="validateNavigation">
        <arquivo :data="{ id_exclusao: register.id }"/>
      </q-tab-panel>
    </q-tab-panels>
...
import { andamentoCogerexclusao, andamentoexclusao, motivoAberturaexclusao, prorogacao, tipoBoletim } from 'src/config/selects'
import { getDense } from 'src/store/utils'
import { getAndamento, getSobrestamento } from 'src/utils'
import { exclusao } from 'src/types'
...
export default defineComponent({
  ...
  setup (_, { refs, root }) {
    const vars = reactive({
      ...
      andamentoCogerexclusao,
      andamentoexclusao,
      motivoAberturaexclusao,
      prorogacao,
      tipoBoletim
    })
    ...
    async function changeAndamento (sobrestamento: { termino_data: string }) {
      ...
      if (!sobrestamento.termino_data) {
        vars.register.id_andamento = getSobrestamento('exclusao')
        const { ok } = await api.put(`exclusao/${id}`, vars.register, { silent: true })
        if (ok) return
      }
      vars.register.id_andamento = getAndamento('exclusao')
      await api.put(`exclusao/${id}`, vars.register, { silent: true })
    }

    async function validateNavigation (tab: string) {
      if (validate(refs, fields)) {
        const { ok } = await api.put(`exclusao/${String(vars.register.id)}`, vars.register, { silent: true })
        if (ok) vars.tab = tab
      } else {
        vars.tab = 'main'
      }
    ...
    async function loadData () {
      ...
      if (id) {
        ...
        if (ok) vars.register = data as exclusao
      }
    }
