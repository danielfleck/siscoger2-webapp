<template>
  <page :breadcrumbs="[
    ...
    { label: 'Criar', link: '/recursos/inserir' },
    ]">
      ...
      <q-step :name="1" title="Dados principais" icon="settings" :done="step > 1">
        <form class="row">
          ...
          <div class="q-pa-md col-4" v-if="register.cdopm">
            <Portaria label="N° Portaria" v-model="register.portaria_numero" ref="portaria_numero" required proc="recursos" :cdopm="register.cdopm"/>
          </div>
          <div class="q-pa-md col-4">
          ...
          <div class="q-pa-md col-6">
            <InputSelect label="Motivo abertura" v-model="register.motivo_abertura" :options="motivoAberturarecursos" />
          </div>
  ...
  setup (_, { refs, root }) {
    ...
    async function create () {
      if (validate(refs, fields)) {
        const { ok, data } = await api.post('recursos', vars.register, { silent: true, debug: true })
        if (ok) {
          const recursos = data as recursos
          vars.register.id = Number(recursos.id)
    ...
    async function update (id: number) {
      if (validate(refs, fields)) {
        const { ok } = await api.put(`recursos/${id}`, vars.register, { silent: true, debug: true })
    ...
    async function finalize () {
      if (validate(refs, fields)) {
        ...
        if (validateSubforms && vars.register.id) {
          ...
          await api.put(`recursos/${vars.register.id}`, vars.register)
          await removePendence(vars.incompleto)
          return root.$router.push('/recursos/lista')
        }
      }
    }
