    ...
    <q-tab-panels v-model="tab" animated>
      <q-tab-panel name="main" class="row">
        ...
        <div class="q-pa-md col-4">
          <Andamento v-model="register.id_andamento" type="recursos"/>
        </div>
        <div class="q-pa-md col-4">
          <AndamentoCoger v-model="register.id_andamentocoger" type="recursos"/>
        </div>
        ...
        <template v-if="register.id">
          <ProcedOrigem type="recursos" :data="{ id_recursos: register.id }"/>
          <Membro label="Sindicante" ref="sindicante" required :data="{ situacao: 'sindicante', id_recursos: register.id }"/>
          <Membro label="Escrivão" ref="escrivao" :data="{ situacao: 'escrivao', id_recursos: register.id }"/>
          <Acusado label="Sindicado" :data="{ situacao: 'sindicado', id_recursos: register.id }"/>
          <Vitima :data="{ id_recursos: register.id }"/>
          <FileUpload label="Relatório do Encarregado" :data="{ proc: 'recursos', campo: 'relatorio_encarregado_file', id_proc: register.id}"/>
          <FileUpload label="Solução do Comandante" :data="{ proc: 'recursos', campo: 'solucao_cmt_file', id_proc: register.id}"/>
          <FileUpload label="Solução CMT Geral" :data="{ proc: 'recursos', campo: 'solucao_cmtgeral_file', id_proc: register.id}"/>
        </template>
        <q-btn @click="update" color="primary" label="Salvar" class="full-width"/>
      </q-tab-panel>

      <q-tab-panel name="movimentos">
        <movimento :data="{ id_recursos: register.id }"/>
      </q-tab-panel>

      <q-tab-panel name="sobrestamentos" @transition="validateNavigation">
        <sobrestamento @submit="changeAndamento" :data="{ id_recursos: register.id }"/>
      </q-tab-panel>

      <q-tab-panel name="arquivos" @transition="validateNavigation">
        <arquivo :data="{ id_recursos: register.id }"/>
      </q-tab-panel>
    </q-tab-panels>
...
import { andamentoCogerrecursos, andamentorecursos, motivoAberturarecursos, prorogacao, tipoBoletim } from 'src/config/selects'
import { getDense } from 'src/store/utils'
import { getAndamento, getSobrestamento } from 'src/utils'
import { recursos } from 'src/types'
...
export default defineComponent({
  ...
  setup (_, { refs, root }) {
    const vars = reactive({
      ...
      andamentoCogerrecursos,
      andamentorecursos,
      motivoAberturarecursos,
      prorogacao,
      tipoBoletim
    })
    ...
    async function changeAndamento (sobrestamento: { termino_data: string }) {
      ...
      if (!sobrestamento.termino_data) {
        vars.register.id_andamento = getSobrestamento('recursos')
        const { ok } = await api.put(`recursos/${id}`, vars.register, { silent: true })
        if (ok) return
      }
      vars.register.id_andamento = getAndamento('recursos')
      await api.put(`recursos/${id}`, vars.register, { silent: true })
    }

    async function validateNavigation (tab: string) {
      if (validate(refs, fields)) {
        const { ok } = await api.put(`recursos/${String(vars.register.id)}`, vars.register, { silent: true })
        if (ok) vars.tab = tab
      } else {
        vars.tab = 'main'
      }
    ...
    async function loadData () {
      ...
      if (id) {
        ...
        if (ok) vars.register = data as recursos
      }
    }
