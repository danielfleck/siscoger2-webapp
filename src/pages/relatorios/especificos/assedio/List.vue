<template>
  <page :breadcrumbs="[
    { label: 'Lista', link: '/abuso' },
    ]">
    <q-btn data-cy="button" color="primary" icon="fa fa-plus" class="full-width" label="Inserir novo" to="/abuso/inserir"/>
    <Table
      data-cy="table"
      label="Lista abuso"
      :data="data"
  ...
  setup (_, { root }) {
    ...
    const functions = {
      ...
      onEdit (row: { id: number }) {
        return root.$router.push(`/abuso/editar/${String(row.id)}`)
      },
      onDelete (row: { id: number }) {
        root.$q.dialog(confirmMsg).onOk(async () => {
          const { ok } = await api.delete(`abuso/${String(row.id)}`)
