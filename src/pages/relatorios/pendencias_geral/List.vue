<template>
  <page :breadcrumbs="[
    ...
    <q-btn data-cy="button" color="primary" icon="fa fa-plus" class="full-width" label="Inserir novo" to="/pendencias_geral/inserir"/>
    <Table
      data-cy="table"
      label="Lista pendencias_geral"
      :data="data"
      :columns="columns"
      actions
      @delete="onDelete"
  ...
  setup (_, { root }) {
    ...
    const functions = {
      ...
      onEdit (row: { id: number }) {
        return root.$router.push(`/pendencias_geral/editar/${String(row.id)}`)
      },
      onDelete (row: { id: number }) {
        root.$q.dialog(confirmMsg).onOk(async () => {
          const { ok } = await api.delete(`pendencias_geral/${String(row.id)}`)
